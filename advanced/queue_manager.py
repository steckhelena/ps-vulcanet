...
class CallCenterProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        if request['command'] == 'call':
            response = self.call_center.receiveCall(request['id'])
        if request['command'] == 'answer':
            response = self.call_center.answerCall(request['id'])
        if request['command'] == 'reject':
            response = self.call_center.rejectCall(request['id'])
        if request['command'] == 'hangup':
            response = self.call_center.hangupCall(request['id'])

        self.sendLine(json.dumps({'response': response}).encode('ascii'))
...
class CallCenterFactory(Factory):
    ...
    def startFactory(self):
        self.call_center = CallCenter(self.operators)
